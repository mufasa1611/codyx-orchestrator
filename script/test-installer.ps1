...
Assert-Contains "script/launcher.ps1" 'Get-CodyxSparseCheckoutPaths[\s\S]/packages/codyx/[\s\S]/packages/app/[\s\S]/patches/[\s\S]!\*\*/\*\.test\.ts' "Standalone launcher slim checkout keeps runtime workspaces and excludes tests"
Assert-NotContains "script/launcher.ps1" '"/packages/(gitlab-auth|poe-auth|script)/"' "Standalone launcher slim checkout excludes optional auth and developer script workspaces"
...
Assert-Contains "script/launcher.ps1" '\$env:HUSKY = "0"[\s\S]*"install", "--force"' "Standalone launcher disables Husky hooks during dependency refresh"
Assert-Contains "script/install.ps1" 'Get-CodyxSparseCheckoutPaths[\s\S]/packages/codyx/[\s\S]/packages/app/[\s\S]/patches/[\s\S]!\*\*/\*\.test\.ts' "Windows installer slim checkout keeps runtime workspaces and excludes tests"
Assert-NotContains "script/install.ps1" '"/packages/(gitlab-auth|poe-auth|script)/"' "Windows installer slim checkout excludes optional auth and developer script workspaces"
...
Assert-NotContains "packages/codyx/package.json" '@cody/(gitlab-auth|poe-auth|script)|@gitlab/opencode-gitlab-auth' "codyx package manifest does not require removed end-user workspaces"
Assert-NotContains "package.json" '@cody/script' "Root package manifest does not require removed developer script workspace"
