...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-cody-session") ?? ""
    const requestId = input.request.headers.get("x-cody-request") ?? ""
    const projectId = input.request.headers.get("x-cody-project") ?? ""
    const ocClient = input.request.headers.get("x-cody-client") ?? ""
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-cody-request")
          headers.delete("x-cody-session")
          headers.delete("x-cody-project")
          headers.delete("x-cody-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://cody.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://cody.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://cody.ai/workspace/${authInfo.workspaceID}/members`
