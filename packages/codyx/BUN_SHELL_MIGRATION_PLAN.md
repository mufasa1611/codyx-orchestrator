...
Replace runtime Bun shell template-tag usage in `packages/cody/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/cody/src` is removed except:
