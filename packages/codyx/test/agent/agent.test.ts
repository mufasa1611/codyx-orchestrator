...
test("plan agent denies edits except .cody/plans/*", async () => {
  await using tmp = await tmpdir()
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".cody/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".cody", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  const home = process.env.CODY_TEST_HOME
  process.env.CODY_TEST_HOME = tmp.path
  ...
  try {
    await WithInstance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".cody", "skill", "perm-skill")
