...
import { CodyClient, type GlobalEvent } from "@cody/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<CodyClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<CodyClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<CodyClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<CodyClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<CodyClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: CodyClient["event"]["subscribe"]
    globalEvent?: CodyClient["global"]["event"]
    promptAsync?: CodyClient["session"]["promptAsync"]
    status?: CodyClient["session"]["status"]
    messages?: CodyClient["session"]["messages"]
    children?: CodyClient["session"]["children"]
    permissions?: CodyClient["permission"]["list"]
    questions?: CodyClient["question"]["list"]
  } = {},
) {
  const client = new CodyClient()

  const subscribe: CodyClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: CodyClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: CodyClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: CodyClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: CodyClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: CodyClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: CodyClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: CodyClient["question"]["list"] = input.questions ?? (() => ok([]))
