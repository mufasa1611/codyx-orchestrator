...
- `config?: Partial<Config.Info>` - Write an `cody.json` config file
...
- Directories are created in the system temp folder with prefix `cody-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("cody-test-")
  }),
)
```
