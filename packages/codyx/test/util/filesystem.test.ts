...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "cody.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "cody.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cody.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cody.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["cody.json", "cody.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "cody.json"),
        path.join(tmp.path, "cody.jsonc"),
        path.join(project, "cody.json"),
        path.join(project, "cody.jsonc"),
      ])
    })
  })
