...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CODY_TEST ?? "")'), {
      env: {
        CODY_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CODY_TEST_SHELL"], {
      shell: true,
      env: {
        CODY_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CODY_TEST_SHELL=ok")
  })
