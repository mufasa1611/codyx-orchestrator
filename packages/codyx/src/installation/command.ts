...
export function createWindowsRemoteUninstallScript(input: {
  ...
  const script = [
    ...
    `  # Try per-file deletion first (works even if root dir is locked)`,
    `  try {`,
    `    Get-ChildItem -LiteralPath $target -Recurse -File -ErrorAction Stop | Sort-Object FullName -Descending | ForEach-Object { Remove-Item -LiteralPath $_.FullName -Force -ErrorAction SilentlyContinue }`,
    `    Get-ChildItem -LiteralPath $target -Recurse -Directory -ErrorAction Stop | Sort-Object FullName -Descending | ForEach-Object { Remove-Item -LiteralPath $_.FullName -Force -ErrorAction SilentlyContinue }`,
    ...
    `  try {`,
    `    Get-ChildItem -LiteralPath $graveyard -Recurse -File -ErrorAction Stop | Sort-Object FullName -Descending | ForEach-Object { Remove-Item -LiteralPath $_.FullName -Force -ErrorAction SilentlyContinue }`,
    `    Remove-Item -LiteralPath $graveyard -Recurse -Force -ErrorAction Stop`,
    `  } catch {}`,
    `}`,
