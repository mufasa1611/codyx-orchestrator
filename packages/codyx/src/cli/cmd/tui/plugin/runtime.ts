...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".cody", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved }) {
  ...
  try {
    await WithInstance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.CODY_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.CODY_PURE && config.plugin_origins?.length) {
