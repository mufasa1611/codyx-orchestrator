...
import type { AssistantMessage, Part, Provider, ToolPart, UserMessage, TextPart, ReasoningPart } from "@cody/sdk/v2"
...
import { usePolicyBan } from "../../context/policy-ban"
...
export function Session() {
  ...
  const policyBan = usePolicyBan()
  const visible = createMemo(() => !session()?.parentID && permissions().length === 0 && questions().length === 0)
  const disabled = createMemo(
    () => permissions().length > 0 || questions().length > 0 || policyBan.isBanned(route.sessionID),
  )
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}codyx -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
