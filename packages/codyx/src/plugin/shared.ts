...
export const DEPRECATED_PLUGIN_PACKAGES = ["cody-openai-codex-auth", "cody-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, codyVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(codyVersion) || semver.major(codyVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.cody
  if (typeof range !== "string") return
  if (!semver.satisfies(codyVersion, range)) {
    throw new Error(`Plugin requires cody ${range} but running ${codyVersion}`)
  }
}
