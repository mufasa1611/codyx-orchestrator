...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const currentUserID = Effect.fn("Workspace.currentUserID")(function* () {
      return yield* UserRef
    })
    const workspaceOwner = (userID: string | undefined) => (userID ? eq(WorkspaceTable.user_id, userID) : undefined)
    ...
    const list = Effect.fn("Workspace.list")(function* (project: Project.Info) {
      ...
      return yield* db((db) =>
        db
          .select()
          .from(WorkspaceTable)
          .where(and(eq(WorkspaceTable.project_id, project.id), workspaceOwner(userID)))
          .all()
          .map(fromRow)
    ...
    const get = Effect.fn("Workspace.get")(function* (id: WorkspaceID) {
      ...
      const row = yield* db((db) =>
        db
          .select()
          .from(WorkspaceTable)
          .where(and(eq(WorkspaceTable.id, id), workspaceOwner(userID)))
          .get(),
      )
      if (!row) return
      return fromRow(row)
    ...
    const status = Effect.fn("Workspace.status")(function* () {
      const userID = yield* currentUserID()
      if (!userID) return [...connections.values()]
      const rows = yield* db((db) =>
        db.select({ id: WorkspaceTable.id }).from(WorkspaceTable).where(eq(WorkspaceTable.user_id, userID)).all(),
      )
      const allowed = new Set(rows.map((row) => row.id))
      return [...connections.values()].filter((item) => allowed.has(item.workspaceID))
    })
    ...
    const startWorkspaceSyncing = Effect.fn("Workspace.startWorkspaceSyncing")(function* (projectID: ProjectID) {
      ...
      const rows = yield* db((db) =>
        db
          ...
          .from(WorkspaceTable)
          .where(and(eq(WorkspaceTable.project_id, projectID), workspaceOwner(userID)))
          .all(),
      )
