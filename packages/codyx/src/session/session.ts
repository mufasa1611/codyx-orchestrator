...
export const Info = Schema.Struct({
  ...
  slug: Schema.String,
  projectID: ProjectID,
  userID: optionalOmitUndefined(Schema.String),
...
export const layer: Layer.Layer<Service, never, Bus.Service | Storage.Service | SyncEvent.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      const userID = input.userID ?? (yield* UserRef)
      const result: Info = {
    ...
    const get = Effect.fn("Session.get")(function* (id: SessionID) {
      ...
      const row = yield* db((d) =>
        d
          .select()
          .from(SessionTable)
          .where(userID ? and(eq(SessionTable.id, id), eq(SessionTable.user_id, userID)) : eq(SessionTable.id, id))
          .get(),
      )
    ...
    const children = Effect.fn("Session.children")(function* (parentID: SessionID) {
      yield* get(parentID).pipe(Effect.orDie)
      const userID = yield* UserRef
    ...
    const updateMessage = <T extends MessageV2.Info>(msg: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        yield* get(msg.sessionID).pipe(Effect.orDie)
    ...
    const updatePart = <T extends MessageV2.Part>(part: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        yield* get(part.sessionID).pipe(Effect.orDie)
    ...
    const getPart: Interface["getPart"] = Effect.fn("Session.getPart")(function* (input) {
      yield* get(input.sessionID).pipe(Effect.orDie)
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      if (input?.parentID) yield* get(input.parentID).pipe(Effect.orDie)
    ...
    const touch = Effect.fn("Session.touch")(function* (sessionID: SessionID) {
      yield* get(sessionID).pipe(Effect.orDie)
