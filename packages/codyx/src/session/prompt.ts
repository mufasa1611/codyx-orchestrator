    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.run")(
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          yield* sessions.updateMessage(msg)
          const failAssistant = Effect.fn("SessionPrompt.failAssistant")(function* (
            message: MessageV2.Assistant,
            error: unknown,
          ) {
            const parsed = MessageV2.fromError(error, { providerID: model.providerID })
            ...
            yield* status.set(message.sessionID, { type: "idle" })
          })
          ...
          if (Exit.isFailure(handleExit)) {
            yield* failAssistant(msg, Cause.squash(handleExit.cause))
            break
          }
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            Effect.catchCause((cause) =>
              Effect.gen(function* () {
                yield* failAssistant(handle.message, Cause.squash(cause))
                return "break" as const
              }),
            ),
