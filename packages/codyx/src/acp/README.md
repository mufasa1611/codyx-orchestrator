...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for cody.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal cody sessions
...
```bash
CODY_ENABLE_QUESTION_TOOL=1 cody acp
```
...
- **Terminal Integration**: Full terminal support via cody's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | cody acp
```
...
ACP sessions map cleanly to cody's internal session model:
