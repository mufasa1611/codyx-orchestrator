...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const currentUserID = Effect.fn("Project.currentUserID")(function* () {
      return yield* UserRef
    })

    const scopedProjectID = (id: ProjectID, userID: string | undefined) =>
      userID ? ProjectID.make(`${id}:user:${userID}`) : id

    const projectOwner = (userID: string | undefined) => (userID ? eq(ProjectTable.user_id, userID) : undefined)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const projectID = scopedProjectID(data.id, userID)
      ...
      if (data.id !== ProjectID.global) {
        yield* db((d) =>
          d
            .update(SessionTable)
            .set({ project_id: result.id })
            .where(
              and(
                eq(SessionTable.project_id, scopedProjectID(ProjectID.global, userID)),
                eq(SessionTable.directory, data.worktree),
                userID ? eq(SessionTable.user_id, userID) : undefined,
              ),
            )
            .run(),
        )
    ...
    const list = Effect.fn("Project.list")(function* () {
      const userID = yield* currentUserID()
      return yield* db((d) =>
        d.select().from(ProjectTable).where(projectOwner(userID)).all().map(fromRow),
      )
    })
    ...
    const get = Effect.fn("Project.get")(function* (id: ProjectID) {
      ...
      const row = yield* db((d) =>
        d
          .select()
          .from(ProjectTable)
          .where(and(eq(ProjectTable.id, id), projectOwner(userID)))
          .get(),
      )
    ...
    const update = Effect.fn("Project.update")(function* (input: UpdateInput) {
      ...
      const result = yield* db((d) =>
        d
          ...
          .where(and(eq(ProjectTable.id, input.projectID), projectOwner(userID)))
          .returning()
          .get(),
      )
