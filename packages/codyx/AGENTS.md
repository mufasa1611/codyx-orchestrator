...
- **Read `.cody/session-log.md` at the start of every session** (before any other task). This file survives conversation compaction and contains active task state, completed tasks, and current goals.
- Update `.cody/session-log.md` after completing tasks (mark done, add to completed section).
- The session log lives at the repo root (`X:\codyx\.cody\session-log.md`), not in the package directory.
