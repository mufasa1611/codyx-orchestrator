...
Hay dos tipos de agentes en Cody; agentes primarios y subagentes.
...
Cody viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Cody viene con dos subagentes integrados, **General** y **Explore**. Veremos esto a continuación.

---

## Integrados

Cody viene con dos agentes primarios integrados y dos subagentes integrados.
...
Configure agentes en su archivo de configuración `cody.json`:
...
Si no se especifica ninguna temperatura, Cody utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Cody como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Cody usa el formato `provider/model-id`. Por ejemplo, si usa [Cody Zen](/docs/zen), usaría `cody/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `cody models` para ver una lista de los modelos disponibles.
:::
