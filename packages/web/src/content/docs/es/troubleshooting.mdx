...
Para depurar problemas con Cody, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\cody\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `cody --log-level DEBUG`.

---

## Almacenamiento

cody almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/cody/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\cody`
...
Cody Desktop ejecuta un servidor Cody local (el sidecar `cody-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Cody` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/cody/cody.jsonc` (o `~/.config/cody/cody.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/cody/cody.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\cody\cody.jsonc`
...
Cody también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/cody/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\cody\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Cody pueda reconstruirlo.

1. Salga de Cody Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/cody`
- **Linux**: eliminar `~/.cache/cody` (o ejecutar `rm -rf ~/.cache/cody`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\cody`

3. Reinicie el escritorio Cody.

---

### Solucionar problemas de conexión del servidor

Cody Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `cody.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio Cody requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Cody.

---

### Notificaciones que no se muestran

Cody Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Cody en la configuración de su sistema operativo, y
...
1. Salga del escritorio Cody.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Cody):

- `cody.settings.dat` (URL del servidor predeterminado de escritorio)
- `cody.global.dat` y `cody.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `cody upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `cody models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\cody`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. cody instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\cody`

2. Reinicie cody para reinstalar los paquetes del proveedor más recientes.

Esto obligará a cody a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
cody detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
