...
Las habilidades del agente permiten a Cody descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Cody busca estas ubicaciones:

- Configuración del proyecto: `.cody/skills/<name>/SKILL.md`
- Configuración global: `~/.config/cody/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Cody sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
...
Las definiciones globales también se cargan desde `~/.config/cody/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Cody enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `cody.json`:
...
**Para agentes integrados** (en `cody.json`):
