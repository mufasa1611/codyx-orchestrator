---
title: Complementos
description: Escriba sus propios complementos para extender Cody.
---

Los complementos le permiten extender Cody al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Cody.
...
- `~/.config/cody/plugins/` - Complementos globales
...
```json title="cody.json"
{
  ...
  "plugin": ["cody-helicone-session", "cody-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/cody/node_modules/`.
...
1. Configuración global (`~/.config/cody/cody.json`)
2. Configuración del proyecto (`cody.json`)
3. Directorio global de complementos (`~/.config/cody/plugins/`)
...
Cody ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK cody para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar cody.
...
:::note
Si está utilizando la aplicación de escritorio Cody, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que cody lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a cody:
...
El ayudante `tool` crea una herramienta personalizada a la que cody puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para cody junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://cody.ai/docs/sdk) para obtener más detalles.
