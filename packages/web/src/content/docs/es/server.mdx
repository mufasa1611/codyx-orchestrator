---
title: Servidor
description: Interactuar con el servidor cody a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `cody serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente cody puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `cody.local` |
...
Configure `CODY_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `cody`, o configure `CODY_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `cody serve` como a `cody web`.
...
Cuando ejecuta `cody`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor cody para interactuar con cody mediante programación.
:::

Esta arquitectura permite que cody admita múltiples clientes y le permite interactuar con cody mediante programación.

Puede ejecutar `cody serve` para iniciar un servidor independiente. Si tienes el
cody TUI ejecutándose, `cody serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Cody [IDE](/docs/ide).
...
El servidor cody expone las siguientes API.
