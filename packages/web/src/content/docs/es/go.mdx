...
Cody Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
Cody Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en Cody. Te suscribes a Cody Go y
...
Cody Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Cody Go funciona como cualquier otro proveedor en Cody.

1. Inicias sesión en **<a href={console}>Cody Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Cody Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Cody Go.
:::
...
Cody Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Cody
usa el formato `cody-go/<model-id>`. Por ejemplo, para Kimi K2.6, usarías
`cody-go/kimi-k2.6` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Cody.
