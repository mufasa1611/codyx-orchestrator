...
For at fejlfinde problemer med Cody, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\cody\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `cody --log-level DEBUG`.

---

## Lagring

cody gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/cody/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\cody`
...
Cody Desktop kører en lokal Cody-server (`cody-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Cody`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/cody/cody.jsonc` (eller `~/.config/cody/cody.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/cody/cody.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\cody\cody.jsonc`
...
Cody kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/cody/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\cody\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Cody kan genopbygge den.

1. Afslut Cody Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/cody`
- **Linux**: slet `~/.cache/cody` (eller kør `rm -rf ~/.cache/cody`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\cody`

3. Start Cody Desktop på ny.

---

### Løs problemer med serverforbindelse

Cody Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `cody.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver Cody Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Cody.

---

### Meddelelser vises ikke

Cody Desktop viser kun systemvarsler når:

- varsler er aktiveret for Cody i dine OS-indstillinger, og
...
1. Afslut Cody Desktop.
2. Find og slet disse filer (de findes i Cody Desktop-appens datamappe):

- `cody.settings.dat` (skrivebordsstandardserver URL)
- `cody.global.dat` og `cody.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Cody:
...
For at finde ud af hvilke modeller du har adgang til, kør `cody models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\cody`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. cody installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\cody`

2. Start cody på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge cody til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
cody vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
