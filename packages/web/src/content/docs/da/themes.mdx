...
Med Cody kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Cody vårt eget `cody`-tema.
...
Cody kommer med flere innebygde temaer.
...
- Vil at Cody skal matche terminalens utseende
...
Cody støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/cody/themes/*.json` eller `$XDG_CONFIG_HOME/cody/themes/*.json`
