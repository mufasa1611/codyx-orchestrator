---
title: Server
description: Interager med cody-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `cody serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en cody-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `cody.local` |
...
Indstil `CODY_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `cody`, eller indstil `CODY_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `cody serve` og `cody web`.
...
Når du kører `cody` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug cody-serveren til at interagere med cody programmatisk.
:::

Denne arkitektur lader cody understøtte flere klienter og lader dig interagere med cody programmatisk.

Du kan køre `cody serve` for at starte en selvstændig server. Hvis du har
cody TUI kørende, vil `cody serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Cody [IDE](/docs/ide) plugins.
...
Cody-serveren viser følgende API'er.
