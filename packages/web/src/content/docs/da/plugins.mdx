---
title: Plugins
description: Skriv dine egne plugins for at udvide Cody.
---

Plugins giver dig mulighed for at udvide Cody ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Codys standardadfærd.
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/cody/node_modules/`.
...
1. Global konfiguration (`~/.config/cody/cody.json`)
2. Projektkonfiguration (`cody.json`)
3. Global plugin-mappe (`~/.config/cody/plugins/`)
...
Cody kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En cody SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide cody.
...
:::note
Hvis du bruger Cody desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå cody i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til cody:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som cody kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for cody sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://cody.ai/docs/sdk) for detaljer.
