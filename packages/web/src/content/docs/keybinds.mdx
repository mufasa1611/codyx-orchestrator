...
Cody customizes TUI keyboard shortcuts with `keymap` in `tui.json`.

The older `keybinds` field is still accepted as a migration fallback, but it is deprecated and will be removed in Cody v2.0. If `keymap` is present, Cody ignores `keybinds` for shortcut resolution.
...
Cody uses a `leader` key for many shortcuts. This avoids conflicts in your terminal.
...
| `leader_timeout` | How long Cody waits for the next key after the leader key, in milliseconds. Defaults to `2000`. |
...
The Cody desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `cody.json`.
