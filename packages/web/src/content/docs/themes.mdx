...
With Cody you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Cody uses our own `cody` theme.
...
Cody comes with several built-in themes.
...
- Want Cody to match their terminal's appearance
...
Cody supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/cody/themes/*.json` or `$XDG_CONFIG_HOME/cody/themes/*.json`
