---
title: TUI
description: Using the Cody terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Cody provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Cody starts the TUI for the current directory.
...
When using the Cody TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Cody. Allows you to select from available providers and add their API keys.
...
This is separate from `cody.json`, which configures server/runtime behavior.
