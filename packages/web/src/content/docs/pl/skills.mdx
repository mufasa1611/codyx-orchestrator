...
Umiejętności agenta pozwalają cody odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
cody przeszukuje te lokalizacje:

- Project config: `.cody/skills/<name>/SKILL.md`
- Global config: `~/.config/cody/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu cody przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
...
Ładowane są także definicje globalne z `~/.config/cody/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
cody wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `cody.json`:
...
**Dla agentów wbudowanych** (w `cody.json`):
