...
Dzięki cody możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie cody używa naszego własnego motywu `cody`.
...
cody ma kilka wbudowanych motywów.
...
- Chcą, aby Cody pasował do wyglądu ich terminala
...
cody obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/cody/themes/*.json` lub `$XDG_CONFIG_HOME/cody/themes/*.json`
