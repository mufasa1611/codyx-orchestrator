---
title: Konfiguracja
description: Ustawienia Cody w pliku JSON.
---

Możesz dostosować Cody za pomocą pliku konfiguracyjnego JSON.

---

## Format

Cody obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
1. **Konfiguracja zdalna** (z `.well-known/cody`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/cody/cody.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `CODY_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`cody.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.cody`** - agenci, polecenia, umiejętności
...
:::note
W katalogach `.cody` i `~/.config/cody` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/cody`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Cody w `~/.config/cody/cody.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/cody/tui.json`.
...
Dodaj `cody.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Cody się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `CODY_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.cody` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.cody`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`cody.ai/config.json`**](https://cody.dev/config.json).
...
Przestarzałe klucze `theme`, `keybinds` i `tui` w `cody.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `cody serve` i `cody web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Cody.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `cody.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Cody wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/cody/agents/` lub `.cody/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Cody powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`cody run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/cody/commands/` lub `.cody/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Cody może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Cody nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Cody **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Cody o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.cody/plugins/` lub `~/.config/cody/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Cody tylko do określonych dostawców.
