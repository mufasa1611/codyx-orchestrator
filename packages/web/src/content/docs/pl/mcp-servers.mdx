...
Możesz dodać dodatkowe informacje do cody za pomocą _Model Context Protocol_, w skrócie MCP. cody obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [cody Config](https://cody.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/cody`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
cody automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, cody:
...
Jeśli serwer wymaga uwierzytelnienia, cody poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `cody mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji cody bezpieczne przechowa tokeny w `~/.local/share/cody/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w cody, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie cody, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć cody z kontem Sentry.
