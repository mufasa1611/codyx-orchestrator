---
title: Serwer
description: Interakcja z serwerem cody za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `cody serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient cody.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `cody.local` |
...
Ustaw `CODY_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `cody` lub ustaw `CODY_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `cody serve`, jak i `cody web`.
...
Kiedy uruchomisz `cody`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera cody do programowej interakcji z cody.
:::

Ta architektura umożliwia obsługę wielu klientów przez cody i programową interakcję z kodem otwartym.

Możesz uruchomić `cody serve`, aby uruchomić samodzielny serwer. Jeśli masz
cody TUI działa, `cody serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki cody [IDE](/docs/ide).
...
Serwer cody udostępnia następujące interfejsy API.
