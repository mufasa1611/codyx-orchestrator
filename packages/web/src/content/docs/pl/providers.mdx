---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w cody.
---

import config from "../../../../config.mjs"
export const console = config.console

cody używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji cody.
...
w `~/.local/share/cody/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim cody
config.
...
Cody Zen to lista modeli dostarczonych przez zespół cody, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z cody. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Cody Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Cody Zen` i przejdź do [cody.ai/auth](https://cody.ai/zen).
...
Działa jak każdy inny dostawca w cody i jest całkowicie opcjonalny w użyciu.

---

## Cody Go

Cody Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół cody, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z cody.

1. Uruchom polecenie `/connect` w TUI, wybierz `Cody Go` i przejdź do [cody.ai/auth](https://cody.ai/zen).
...
Aby używać Amazon Bedrock z cody:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania cody:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `cody.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w cody nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować cody do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod cody działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji cody.
...
cody używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować cody
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`cody.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem cody:
...
Aby używać Google Vertex AI z cody:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania cody.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą cody, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [cody-helicone-session](https://github.com/H2Shami/cody-helicone-session), aby automatycznie rejestrować każdą konwersację cody jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja cody będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować cody tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować cody tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować cody tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem cody. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/cody).
:::
...
Aby korzystać z Ollama Cloud z cody:
...
3. Skopiuj klucz API do użycia w cody.
...
6. **Ważne**: Przed użyciem modeli chmurowych w cody musisz pobrać informacje o modelu lokalnie:
...
Cody Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół cody. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Cody Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Cody Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API cody.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji cody.
...
5. Możesz je także dostosować za pomocą konfiguracji cody. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z cody:
...
Możesz także dostosować modele za pomocą konfiguracji cody. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z cody. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `cody.json` w katalogu projektu:
...
Pola `limit` pozwalają cody zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `cody auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację cody i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji cody.
