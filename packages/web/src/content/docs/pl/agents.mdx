...
W Cody obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Cody zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Cody ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Cody ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `cody.json`:
...
Jeśli nie zostanie skonfigurowana, Cody używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Cody, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Cody w `provider/model-id`. Na przykład, [Cody Zen](/docs/zen), użyj `cody/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `cody models`, aby zobaczyć dostępne modele.
:::
