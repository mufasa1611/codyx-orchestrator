...
description: Skriv dine egne programtillegg for å utvide Cody.
---

Plugins lar deg utvide Cody ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Cody.
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/cody/node_modules/`.
...
1. Global konfigurasjon (`~/.config/cody/cody.json`)
2. Prosjektkonfigurasjon (`cody.json`)
3. Global plugin-katalog (`~/.config/cody/plugins/`)
...
Cody kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Cody SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Cody.
...
:::note
Hvis du bruker Cody-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Cody fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Cody:
...
`tool`-hjelperen lager et tilpasset verktøy som Cody kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Cody sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://cody.ai/docs/sdk) for detaljer.
