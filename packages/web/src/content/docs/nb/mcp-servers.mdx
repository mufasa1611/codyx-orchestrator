...
Du kan legge til eksterne verktøy i Cody ved å bruke _Model Context Protocol_ eller MCP. Cody støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Cody Config](https://cody.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="cody.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/cody`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Cody håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Cody:
...
Hvis serveren krever autentisering, vil Cody be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `cody mcp auth <server-name>`.
...
```bash
cody mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Cody lagre tokens sikkert i `~/.local/share/cody/mcp-auth.json`.
...
cody mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Cody, sammen med innebygde verktøy. Så du kan administrere dem gjennom Cody-konfigurasjonen som alle andre verktøy.
...
```bash
cody mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Cody til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Cody-samtale oppført som en separat økt.
