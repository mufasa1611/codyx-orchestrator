...
Det er to typer agenter i Cody; primære agenter og underagenter.
...
Cody kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Cody kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Cody kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `cody.json`:
...
Hvis ingen temperatur er spesifisert, bruker Cody modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Cody-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Cody-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Cody Zen](/docs/zen), vil du bruke `cody/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `cody models` for å se en liste over tilgjengelige modeller.
:::
