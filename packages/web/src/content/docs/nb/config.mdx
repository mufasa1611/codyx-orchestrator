---
title: Konfigurasjon
description: Ved å bruke cody JSON-konfigurasjonen.
---

Du kan konfigurere Cody ved å bruke en JSON konfigurasjonsfil.

---

## Format

Cody støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Ekstern konfigurasjon** (fra `.well-known/cody`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/cody/cody.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`CODY_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`cody.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.cody` kataloger** - agenter, kommandoer, plugins
...
:::note
`.cody` og `~/.config/cody` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/cody`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Cody-konfigurasjon i `~/.config/cody/cody.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `cody.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Cody starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
moduser og plugins akkurat som standard `.cody` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.cody`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`cody.ai/config.json`**](https://cody.dev/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `cody serve` og `cody web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Cody-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `cody.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Cody-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Cody å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/cody/agents/` eller `.cody/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Cody falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`cody run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/cody/commands/` eller `.cody/commands/`. [Les mer her](/docs/commands).
...
Cody vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Cody alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Cody med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.cody/plugins/` eller `~/.config/cody/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Cody til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
