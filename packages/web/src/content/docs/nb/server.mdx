---
title: Tjener
description: Samhandle med Cody-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `cody serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Cody-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `cody.local` |
...
Sett `CODY_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `cody` som standard, eller sett `CODY_SERVER_USERNAME` for å overstyre det. Dette gjelder både `cody serve` og `cody web`.
...
Når du kjører `cody` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Cody-serveren til å samhandle med Cody programmatisk.
:::

Denne arkitekturen lar Cody støtte flere klienter og lar deg samhandle med Cody programmatisk.

Du kan kjøre `cody serve` for å starte en frittstående server. Hvis du har
Cody TUI kjører, vil `cody serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Cody [IDE](/docs/ide) plugins.
...
cody-serveren viser følgende APIer.
