...
_Model Bağlam Protokolü_ veya MCP kullanarak cody'a harici araçlar ekleyebilirsiniz. cody hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [cody Config](https://cody.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/cody` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
cody, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde cody şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa cody, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `cody mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından cody, belirteçleri `~/.local/share/cody/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra cody'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi cody yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve cody'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
