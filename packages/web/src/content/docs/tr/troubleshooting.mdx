...
cody ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\cody\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `cody --log-level DEBUG`.

---

## Depolama

cody, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/cody/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\cody` yapistirin
...
cody Desktop arka planda yerel bir cody sunucusu (`cody-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `cody` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/cody/cody.jsonc` (veya `~/.config/cody/cody.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/cody/cody.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\cody\cody.jsonc` yapistirin
...
cody diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/cody/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\cody\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), cody'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. cody Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/cody`
- **Linux**: `~/.cache/cody` dizinini silin (veya `rm -rf ~/.cache/cody` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\cody` yapistirin

3. cody Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

cody Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`cody.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta cody Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, cody ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

cody Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda cody icin bildirimler etkinse
...
1. cody Desktop'u kapatin
2. Su dosyalari bulun ve silin (cody Desktop uygulama veri dizininde yer alirlar):

- `cody.settings.dat` (desktop varsayilan sunucu URL'si)
- `cody.global.dat` ve `cody.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
cody ile ilgili bir sorun yasiyorsaniz:
...
3. `cody upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `cody models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\cody`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. cody, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\cody`

2. En guncel provider paketlerini yeniden kurmak icin cody'u yeniden baslatin

Bu, cody'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
cody, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
