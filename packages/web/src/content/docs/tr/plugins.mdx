---
title: Eklentiler
description: Cody'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Cody'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Cody'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `~/.config/cody/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/cody/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/cody/cody.json`)
2. Project config (`cody.json`)
3. Global eklenti dizini (`~/.config/cody/plugins/`)
...
Cody bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Cody SDK istemcisi.
...
Cody'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Cody masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Cody'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Cody'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Cody'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Cody'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://cody.ai/docs/sdk) bakın.
