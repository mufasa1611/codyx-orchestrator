...
cody'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
cody, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
cody, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

cody iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `cody.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse cody modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global cody yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
cody yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Cody Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `cody/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `cody models` komutunu çalıştırın.
:::
