...
Cody Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Cody Go şu anda beta olarak mevcut.
:::

Go, Cody'daki diğer sağlayıcılar gibi çalışır. Cody Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Cody'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Cody Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Cody Go, Cody'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Cody Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Cody Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Cody Go'ya abone olabilir.
:::
...
Cody Go aşağıdaki limitleri içerir:
...
Cody yapılandırmanızdaki [model id](/docs/config/#models) formatı `cody-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `cody-go/kimi-k2.6` kullanmalısınız.
...
Cody Go'yu şu amaçlarla oluşturduk:
...
4. Cody ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
