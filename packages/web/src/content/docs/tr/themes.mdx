...
cody ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak cody kendi `cody` temasını kullanır.
...
cody birden fazla yerleşik temayla gelir.
...
- cody'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
cody, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/cody/themes/*.json` veya `$XDG_CONFIG_HOME/cody/themes/*.json`
