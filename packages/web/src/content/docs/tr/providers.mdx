---
title: Sağlayıcılar
description: cody'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

cody, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı cody yapılandırmanızda yapılandırın.
...
`~/.local/share/cody/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları cody'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Cody Zen, cody ekibi tarafından sağlanan modellerin bir listesidir.
cody ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Cody Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, cody'u seçin ve [cody.ai/auth](https://cody.ai/auth)'ye gidin.
...
cody'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Cody Go

Cody Go, cody ile iyi çalıştığı test edilmiş ve doğrulanmış, cody ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Cody Go`'yu seçin ve [cody.ai/auth](https://cody.ai/zen) adresine gidin.
...
Amazon Bedrock'u cody ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   cody'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `cody.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi cody'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
cody'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   cody'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Cody yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in cody auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
cody, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. cody'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`cody.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`cody.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi cody'la kullanmak için:
...
Google Vertex AI'yi cody ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları cody çalıştırılırken ayarlayın.
...
Helicone'un cody aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her cody görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [cody-helicone-session](https://github.com/H2Shami/cody-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her cody görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
cody'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla cody'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla cody'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini cody için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/cody)'a bakın.
:::
...
Ollama Cloud'u cody ile kullanmak için:
...
3. cody'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: cody'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Cody Zen, cody ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Cody Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Cody Zen**'i arayın.

   ```txt
   /connect
   ```

3. cody API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   cody yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca cody yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Cody ile kullanmak için:
...
Modelleri cody yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı cody ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `cody.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları cody'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `cody auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için cody yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, cody yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
