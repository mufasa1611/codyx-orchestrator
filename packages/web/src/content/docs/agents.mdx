...
There are two types of agents in Cody; primary agents and subagents.
...
Cody comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Cody comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Cody comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `cody.json` config file:
...
If no temperature is specified, Cody uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Cody config and the project specific config.
...
The model ID in your Cody config uses the format `provider/model-id`. For example, if you're using [Cody Zen](/docs/zen), you would use `cody/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `cody models` to see a list of the available models.
:::
