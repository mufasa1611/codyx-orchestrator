---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Cody.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Cody fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Cody avvia la TUI per la directory corrente.
...
Quando usi la TUI di Cody, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Cody. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `cody.json`, che configura il comportamento di server/runtime.
