...
In Cody esistono due tipi di agenti: agenti primari e subagenti.
...
Cody include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Cody include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Cody include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `cody.json`:
...
Se non specifichi una temperature, Cody usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Cody sia per la config specifica del progetto.
...
L'ID modello nella configurazione Cody usa il formato `provider/model-id`. Per esempio, se stai usando [Cody Zen](/docs/zen), useresti `cody/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `cody models` per vedere la lista dei modelli disponibili.
:::
