---
title: Server
description: Interagisci con il server cody via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `cody serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client cody.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `cody.local` |
...
Imposta `CODY_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `cody`, oppure imposta `CODY_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `cody serve` sia per `cody web`.
...
Quando esegui `cody` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server cody per interagire con cody in modo programmatico.
:::

Questa architettura permette a cody di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `cody serve` per avviare un server standalone. Se la TUI di cody e gia in esecuzione, `cody serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Cody.
...
Il server cody espone le seguenti API.
