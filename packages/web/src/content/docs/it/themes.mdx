...
Con Cody puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Cody usa il tema `cody`.
...
Cody include diversi temi integrati.
...
- Vuole che Cody corrisponda all'aspetto del terminale
...
Cody supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/cody/themes/*.json` o `$XDG_CONFIG_HOME/cody/themes/*.json`
