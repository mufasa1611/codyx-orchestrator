---
title: Supporto ACP
description: Usa Cody in qualunque editor compatibile con ACP.
---

Cody supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Cody via ACP, configura l'editor in modo che esegua il comando `cody acp`.

Il comando avvia Cody come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Cody' nel selettore agenti di AI Chat.
...
Per usare Cody come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Cody come agente ACP per la chat.
...
Cody funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Cody
