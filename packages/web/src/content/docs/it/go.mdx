...
Cody Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
Cody Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in Cody. Ti abboni a Cody Go e
...
Cody Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Cody Go funziona come qualsiasi altro provider in Cody.

1. Accedi a **<a href={console}>Cody Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Cody Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Cody Go.
:::
...
Cody Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Cody config
utilizza il formato `cody-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`cody-go/kimi-k2.6` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Cody.
