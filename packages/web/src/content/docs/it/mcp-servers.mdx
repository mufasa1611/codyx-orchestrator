...
Puoi aggiungere strumenti esterni a Cody usando il _Model Context Protocol_, o MCP. Cody supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Cody Config](https://cody.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/cody`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Cody gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Cody:
...
Se il server richiede autenticazione, Cody ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `cody mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Cody memorizzera' i token in modo sicuro in `~/.local/share/cody/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Cody insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Cody come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Cody al tuo account Sentry.
