...
Le skill degli agenti permettono a Cody di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Cody cerca in queste posizioni:

- Config di progetto: `.cody/skills/<name>/SKILL.md`
- Config globale: `~/.config/cody/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Cody risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
...
Le definizioni globali vengono caricate anche da `~/.config/cody/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Cody elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `cody.json`:
...
**Per agenti integrati** (in `cody.json`):
