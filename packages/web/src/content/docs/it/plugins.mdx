---
title: Plugin
description: Scrivi plugin per estendere Cody.
---

I plugin ti permettono di estendere Cody agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Cody.
...
- `~/.config/cody/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/cody/node_modules/`.
...
1. Config globale (`~/.config/cody/cody.json`)
2. Config di progetto (`cody.json`)
3. Directory plugin globale (`~/.config/cody/plugins/`)
...
Cody esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di cody per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere cody.
...
:::note
Se usi l'app desktop di Cody, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a cody di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a cody:
...
L'helper `tool` crea uno strumento personalizzato che cody può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in cody insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://cody.ai/docs/sdk) per i dettagli.
