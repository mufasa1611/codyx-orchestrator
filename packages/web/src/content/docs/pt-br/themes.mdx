...
Com o cody, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o cody usa nosso próprio tema `cody`.
...
O cody vem com vários temas embutidos.
...
- Querem que o cody corresponda à aparência do seu terminal
...
O cody suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/cody/themes/*.json` ou `$XDG_CONFIG_HOME/cody/themes/*.json`
