---
title: Configuração
description: Usando a configuração JSON do cody.
---

Você pode configurar o cody usando um arquivo de configuração JSON.

---

## Formato

O cody suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/cody`) - padrões organizacionais
2. **Configuração global** (`~/.config/cody/cody.json`) - preferências do usuário
3. **Configuração personalizada** (`CODY_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`cody.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.cody`** - agentes, comandos, plugins
...
:::note
Os diretórios `.cody` e `~/.config/cody` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/cody`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do cody em `~/.config/cody/cody.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/cody/tui.json`.
...
Adicione `cody.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o cody é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `CODY_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.cody`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.cody`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`cody.ai/config.json`**](https://cody.dev/config.json).

A configuração do TUI usa [**`cody.ai/tui.json`**](https://cody.ai/tui.json).
...
Chaves legadas `theme`, `keybinds` e `tui` em `cody.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `cody serve` e `cody web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor cody.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `cody.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do cody através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o cody tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/cody/agents/` ou `.cody/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o cody recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`cody run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/cody/commands/` ou `.cody/commands/`. [Saiba mais aqui](/docs/commands).
...
O cody fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o cody **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o cody com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.cody/plugins/` ou `~/.config/cody/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o cody para usar apenas provedores específicos, em vez de desabilitá-los um a um.
