...
Você pode adicionar ferramentas externas ao cody usando o _Model Context Protocol_, ou MCP. O cody suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do cody](https://cody.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/cody`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O cody lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o cody irá:
...
Se o servidor requer autenticação, o cody solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `cody mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o cody armazenará os tokens de forma segura em `~/.local/share/cody/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no cody, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do cody como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o cody à sua conta do Sentry.
