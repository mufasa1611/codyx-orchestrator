---
title: Plugins
description: Escreva seus próprios plugins para estender o cody.
---

Plugins permitem que você estenda o cody conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do cody.
...
- `~/.config/cody/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/cody/node_modules/`.
...
1. Configuração global (`~/.config/cody/cody.json`)
2. Configuração do projeto (`cody.json`)
3. Diretório de plugins global (`~/.config/cody/plugins/`)
...
O cody executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do cody para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o cody.
...
:::note
Se você estiver usando o aplicativo desktop cody, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o cody de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao cody:
...
O helper `tool` cria uma ferramenta personalizada que o cody pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o cody junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://cody.ai/docs/sdk) para detalhes.
