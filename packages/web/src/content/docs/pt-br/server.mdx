---
title: Servidor
description: Interaja com o servidor cody via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `cody serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente cody pode usar.
...
```bash
cody serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `cody.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
cody serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CODY_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `cody`, ou defina `CODY_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `cody serve` quanto ao `cody web`.

```bash
CODY_SERVER_PASSWORD=your-password cody serve
```
...
Quando você executa `cody`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor cody para interagir com o cody programaticamente.
:::

Essa arquitetura permite que o cody suporte múltiplos clientes e permite que você interaja com o cody programaticamente.

Você pode executar `cody serve` para iniciar um servidor autônomo. Se você tiver o TUI do cody em execução, `cody serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do cody [IDE](/docs/ide).
...
O servidor cody expõe as seguintes APIs.
