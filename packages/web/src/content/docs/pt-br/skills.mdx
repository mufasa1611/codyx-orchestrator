...
As habilidades do agente permitem que o cody descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O cody pesquisa nesses locais:

- Configuração do projeto: `.cody/skills/<name>/SKILL.md`
- Configuração global: `~/.config/cody/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o cody sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
...
As definições globais também são carregadas de `~/.config/cody/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
O cody lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `cody.json`:
...
**Para agentes embutidos** (em `cody.json`):
