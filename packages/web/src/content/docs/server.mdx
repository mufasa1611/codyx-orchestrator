---
title: Server
description: Interact with cody server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `cody serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an cody client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `cody.local` |
...
Set `CODY_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `cody`, or set `CODY_SERVER_USERNAME` to override it. This applies to both `cody serve` and `cody web`.
...
When you run `cody` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the cody server to interact with cody programmatically.
:::

This architecture lets cody support multiple clients and allows you to interact with cody programmatically.

You can run `cody serve` to start a standalone server. If you have the
cody TUI running, `cody serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Cody [IDE](/docs/ide) plugins.
...
The cody server exposes the following APIs.
