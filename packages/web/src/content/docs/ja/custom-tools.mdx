---
title: カスタムツール
description: LLM が Cody で呼び出すことができるツールを作成します。
---

カスタムツールは、会話中に LLM が呼び出すことができる作成した関数です。これらは、Cody の[組み込みツール](/docs/tools) (`read`、`write`、`bash` など) と連携して動作します。
...
- または、グローバルに、それらを `~/.config/cody/tools/` に配置します。
