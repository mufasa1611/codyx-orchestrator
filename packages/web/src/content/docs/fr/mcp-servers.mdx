...
Vous pouvez ajouter des outils externes à Cody à l'aide du _Model Context Protocol_ ou MCP. Cody prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Cody Config](https://cody.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/cody`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
Cody gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Cody :
...
Si le serveur nécessite une authentification, Cody vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `cody mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Cody stockera les jetons en toute sécurité dans `~/.local/share/cody/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Cody, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Cody comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Cody à votre compte Sentry.
