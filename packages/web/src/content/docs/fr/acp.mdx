---
title: Support ACP
description: Utilisez Cody dans n’importe quel éditeur compatible ACP.
---

Cody prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser Cody via ACP, configurez votre éditeur pour exécuter la commande `cody acp`.

La commande démarre Cody en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "Cody": {
      "command": "/absolute/path/bin/cody",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « Cody » dans le sélecteur d'agent AI Chat.
...
Pour utiliser Cody comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser Cody comme agent ACP pour le chat.
...
Cody fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration Cody
