---
title: Serveur
description: Interagissez avec le serveur cody via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `cody serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client cody peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `cody.local` |
...
Définissez `CODY_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `cody`, ou définissez `CODY_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `cody serve` et à `cody web`.
...
Lorsque vous exécutez `cody`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur cody pour interagir avec cody par programmation.
:::

Cette architecture permet à cody de prendre en charge plusieurs clients et vous permet d'interagir avec cody par programme.

Vous pouvez exécuter `cody serve` pour démarrer un serveur autonome. Si vous avez le
cody TUI en cours d'exécution, `cody serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Cody [IDE](/docs/ide).
...
Le serveur cody expose les API suivantes.
