---
title: SDK
description: Client JS de type sécurisé pour le serveur cody.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK cody JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler cody par programme.
...
Créez une instance de cody :

```javascript
import { createCody } from "@cody/sdk"

const { client } = await createCody()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `cody.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${cody.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de cody, vous pouvez créer une instance client pour vous y connecter :
