...
Avec Cody, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Cody utilise notre propre thème `cody`.
...
Cody est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Cody corresponde à l'apparence de leur terminal
...
Cody prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/cody/themes/*.json` ou `$XDG_CONFIG_HOME/cody/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/cody/themes
vim ~/.config/cody/themes/my-theme.json
```
...
{
  "$schema": "https://cody.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
