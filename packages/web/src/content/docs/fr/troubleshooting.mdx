...
Pour déboguer les problèmes avec Cody, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\cody\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `cody --log-level DEBUG`.

---

## Stockage

cody stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/cody/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\cody`
...
Cody Desktop exécute un serveur Cody local (le side-car `cody-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Cody` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/cody/cody.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\cody\cody.jsonc`
...
Cody peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/cody/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\cody\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Cody puisse le reconstruire.

1. Quittez complètement Cody Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/cody`
- **Linux** : supprimez `~/.cache/cody` (ou exécutez `rm -rf ~/.cache/cody`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\cody`

3. Redémarrez le bureau Cody.

---

### Résoudre les problèmes de connexion au serveur

Cody Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `cody.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, Cody Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.
...
Cody Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Cody dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Cody.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Cody Desktop) :

- `cody.settings.dat` (serveur de bureau par défaut URL)
- `cody.global.dat` et `cody.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Cody :
...
3. Assurez-vous d'avoir la dernière version avec `cody upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `cody models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\cody`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. cody installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\cody`

2. Redémarrez cody pour réinstaller les derniers packages du fournisseur

Cela forcera cody à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
cody détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
