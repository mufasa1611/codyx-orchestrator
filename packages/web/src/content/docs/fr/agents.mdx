...
Il existe deux types d'agents dans Cody ; agents primaires et sous-agents.
...
Cody est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Cody est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Cody est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `cody.json` :
...
- Global : `~/.config/cody/agents/`
- Par projet : `.cody/agents/`

```markdown title="~/.config/cody/agents/review.md"
---
...
Si aucune température n'est spécifiée, Cody utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Cody et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Cody utilise le format `provider/model-id`. Par exemple, si vous utilisez [Cody Zen](/docs/zen), vous utiliserez `cody/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `cody models` pour voir une liste des modèles disponibles.
:::
...
```markdown title="~/.config/cody/agents/docs-writer.md"
---
...
```markdown title="~/.config/cody/agents/security-auditor.md"
---
