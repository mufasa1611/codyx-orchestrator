---
title: Plugins
description: Écrivez vos propres plugins pour étendre Cody.
---

Les plugins vous permettent d'étendre Cody en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Cody.
...
- `~/.config/cody/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/cody/node_modules/`.
...
1. Configuration globale (`~/.config/cody/cody.json`)
2. Configuration du projet (`cody.json`)
3. Répertoire global des plugins (`~/.config/cody/plugins/`)
...
Cody exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK cody pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre cody.
...
:::note
Si vous utilisez l'application de bureau Cody, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez cody de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à cody :
...
L'assistant `tool` crée un outil personnalisé que cody peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour cody aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://cody.ai/docs/sdk) pour plus de détails.
