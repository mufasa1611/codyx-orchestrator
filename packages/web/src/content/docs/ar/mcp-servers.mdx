...
يمكنك إضافة أدوات خارجية إلى Cody باستخدام _Model Context Protocol_، أو MCP. يدعم Cody خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Cody](https://cody.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/cody` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Cody تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Cody بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Cody المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `cody mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Cody الرموز بشكل آمن في `~/.local/share/cody/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Cody إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Cody مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Cody بحساب Sentry الخاص بك.
