---
title: المزوّدون
description: استخدام أي موفّر LLM في Cody.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Cody ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Cody.
...
في `~/.local/share/cody/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Cody.
...
Cody Zen هي قائمة نماذج يوفّرها فريق Cody وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Cody. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Cody Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر cody، ثم انتقل إلى [cody.ai/auth](https://cody.ai/auth).
...
يعمل مثل أي مزوّد آخر في Cody واستخدامه اختياري بالكامل.

---

## Cody Go

Cody Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Cody، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Cody.

1. شغّل الأمر `/connect` في TUI، واختر `Cody Go`، ثم انتقل إلى [cody.ai/auth](https://cody.ai/zen).
...
لاستخدام Amazon Bedrock مع Cody:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل cody:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `cody.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Cody غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين cody لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل cody بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات cody.
...
تخزين الرمز في مساحة تخزين مصادقة cody.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Cody نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Cody
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`cody.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع cody:
...
لاستخدام Google Vertex AI مع Cody:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر cody، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [cody-helicone-session](https://github.com/H2Shami/cody-helicone-session) لتسجيل كل محادثة Cody تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Cody مدرجة كجلسة مستقلة.
...
يمكنك ضبط cody لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط cody لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط cody لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Cody. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/cody) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Cody.
...
6. **مهم**: قبل استخدام نماذج السحابة في Cody، يجب سحب معلومات النموذج محليا:
...
Cody Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Cody. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Cody Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Cody Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Cody.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات cody.
...
5. يمكنك أيضا تخصيصها عبر إعدادات cody. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Cody:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات cody. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع cody. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `cody.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Cody بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `cody auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات cody وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات cody.
