...
مع Cody يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Cody سمتنا `cody`.
...
- يريدون أن يطابق Cody مظهر terminal لديهم
...
يدعم Cody نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/cody/themes/*.json` أو `$XDG_CONFIG_HOME/cody/themes/*.json`
