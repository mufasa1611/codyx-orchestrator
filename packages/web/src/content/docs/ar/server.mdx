---
title: الخادم
description: تفاعل مع خادم cody عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `cody serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل cody استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `cody.local` |
...
عيّن `CODY_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `cody`، أو عيّن `CODY_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `cody serve` و `cody web`.
...
عند تشغيل `cody` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم cody للتفاعل مع cody برمجيا.
:::

تتيح هذه البنية لـ cody دعم عدة عملاء وتمكّنك من التفاعل مع cody برمجيا.

يمكنك تشغيل `cody serve` لبدء خادم مستقل. إذا كانت واجهة cody في terminal (TUI)
قيد التشغيل، فسيبدأ `cody serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Cody لـ [IDE](/docs/ide).
...
يعرض خادم cody واجهات API التالية.
