...
يمكنك إضافة أوامر مخصصة عبر إعدادات Cody أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Cody:
...
- على مستوى النظام: `~/.config/cody/commands/`
...
يتضمن cody عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
