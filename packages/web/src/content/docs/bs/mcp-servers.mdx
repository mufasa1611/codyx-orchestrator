...
Možete dodati vanjske alate u Cody koristeći _Model Context Protocol_, ili MCP. Cody podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Cody Config](https://cody.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/cody`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Cody automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Cody će:
...
Ako server zahtijeva autentifikaciju, Cody će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `cody mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Cody sigurno čuva tokene u `~/.local/share/cody/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Cody, zajedno s ugrađenim alatima. Možete njima upravljati kroz Cody konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Cody sa vašim Sentry nalogom.
