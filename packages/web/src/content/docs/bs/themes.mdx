...
U Cody mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Cody koristi nasu `cody` temu.
...
Cody dolazi sa vise ugradenih tema.
...
- Zele da Cody odgovara izgledu njihovog terminala
...
Cody podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/cody/themes/*.json` ili `$XDG_CONFIG_HOME/cody/themes/*.json`
