---
title: Server
description: Komunicirajte s cody serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `cody serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji cody klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `cody.local` |
...
Postavite `CODY_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `cody`, ili postavite `CODY_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `cody serve` i za `cody web`.
...
Kada pokrenete `cody`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite cody server za programsku interakciju sa cody-om.
:::

Ova arhitektura omogucava cody podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `cody serve` da startate standalone server. Ako je cody TUI vec pokrenut, `cody serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Cody [IDE](/docs/ide) pluginovi.
...
cody server izlaže sljedece API-je.
