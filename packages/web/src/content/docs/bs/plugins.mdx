---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Cody.
---

Dodaci vam omogućavaju da proširite Cody spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Cody.
...
- `~/.config/cody/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/cody/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/cody/cody.json`)
2. Konfiguracija projekta (`cody.json`)
3. Globalni direktorij dodataka (`~/.config/cody/plugins/`)
...
Cody pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Cody SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Cody.
...
Spriječite cody da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u cody:
...
Pomoćnik `tool` kreira prilagođeni alat koji cody može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za cody zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://cody.ai/docs/sdk) za detalje.
