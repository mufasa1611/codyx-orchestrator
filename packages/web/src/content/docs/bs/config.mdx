---
title: Konfiguracija
description: Korištenje Cody JSON konfiguracije.
---

Možete konfigurirati Cody koristeći JSON konfiguracijski fajl.

---

## Format

Cody podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/cody`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/cody/cody.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CODY_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`cody.json` u projektu) - postavke specifične za projekat
5. **`.cody` direktoriji** - agenti, komande, dodaci
...
:::note
Direktoriji `.cody` i `~/.config/cody` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/cody` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Cody konfiguraciju u `~/.config/cody/cody.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/cody/tui.json`.
...
Dodajte `cody.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Cody pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CODY_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.cody` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.cody`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`cody.ai/config.json`**](https://cody.dev/config.json).

TUI konfiguracija koristi [**`cody.ai/tui.json`**](https://cody.ai/tui.json).
...
Stari `theme`, `keybinds`, i `tui` ključevi u `cody.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `cody serve` i `cody web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Cody server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `cody.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Cody konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Cody pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/cody/agents/` ili `.cody/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Cody će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`cody run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/cody/commands/` ili `.cody/commands/`. [Saznajte više ovdje](/docs/commands).
...
Cody će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Cody **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Cody sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.cody/plugins/` ili `~/.config/cody/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Cody da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
