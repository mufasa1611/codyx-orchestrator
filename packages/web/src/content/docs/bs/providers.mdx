---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Cody.
---

import config from "../../../../config.mjs"
export const console = config.console

Cody koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Cody konfiguraciji.
...
u `~/.local/share/cody/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Cody
config.
...
Cody Zen je lista modela koje je obezbedio Cody tim koji su bili
testirano i potvrđeno da dobro radi sa Cody. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Cody Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite cody i idite na [cody.ai/auth](https://cody.ai/auth).
...
Radi kao i svaki drugi provajder u Cody i potpuno je opcionalan za korištenje.

---

## Cody Go

Cody Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Cody tim i koji su testirani i verificirani da dobro rade s Cody-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Cody Go` i idite na [cody.ai/auth](https://cody.ai/zen).
...
Da biste koristili Amazon Bedrock s Cody:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete cody:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `cody.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Cody.
:::
...
Možete konfigurirati cody za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi cody ispravno radio.
   :::
...
Također možete dodati modele kroz svoju cody konfiguraciju.
...
da pohrani token u cody auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Cody koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Cody
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`cody.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s cody:
...
Za korištenje Google Vertex AI s Cody:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće cody.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem codya, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [cody-helicone-session](https://github.com/H2Shami/cody-helicone-session) da automatski prijavite svaki Cody razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Cody razgovor naveden kao zasebna sesija.
...
Možete konfigurirati cody za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati cody za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati cody za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Cody. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/cody) za detalje.
:::
...
Da biste koristili Ollama Cloud s Cody:
...
3. Kopirajte API ključ za korištenje u Cody.
...
6. **Važno**: Prije upotrebe modela oblaka u Cody, morate lokalno povući informacije o modelu:
...
Cody Zen je lista testiranih i verifikovanih modela koju je obezbedio Cody tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Cody Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Cody Zen**.

```txt
   /connect
```

3. Unesite svoj Cody API ključ.
...
Također možete dodati dodatne modele putem vaše cody konfiguracije.
...
5. Također ih možete prilagoditi putem vaše cody konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Codyom:
...
Također možete prilagoditi modele kroz svoju cody konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s cody-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `cody.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Cody da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `cody auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Cody konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj cody konfiguraciji.
