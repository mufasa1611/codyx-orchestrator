...
Postoje dvije vrste agenata u Cody: primarni agenti i podagenti.
...
Cody dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Cody dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Cody dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `cody.json`:
...
Ako temperatura nije navedena, Cody koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Cody konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Cody konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Cody Zen](/docs/zen), koristili biste `cody/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `cody models` da vidite listu dostupnih modela.
:::
