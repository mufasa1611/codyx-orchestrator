...
Agent skills omogucavaju Cody da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Cody pretrazuje ove lokacije:

- Konfiguracija projekta: `.cody/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/cody/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Cody ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.cody/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/cody/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Cody navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `cody.json`:
...
**Za ugradene agente** (u `cody.json`):
