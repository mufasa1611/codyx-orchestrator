...
Da biste otklonili probleme s Cody, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\cody\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `cody --log-level DEBUG`.

---

## Pohrana

Cody pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/cody/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\cody`
...
Cody Desktop pokreće lokalni Cody server (`cody-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Cody` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/cody/cody.jsonc` (ili `~/.config/cody/cody.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/cody/cody.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\cody\cody.jsonc`
...
Cody također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/cody/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\cody\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Cody mogao ponovo izgraditi.

1. Potpuno zatvorite Cody Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/cody`
- **Linux**: obrišite `~/.cache/cody` (ili pokrenite `rm -rf ~/.cache/cody`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\cody`

3. Ponovo pokrenite Cody Desktop.

---

### Rješavanje problema sa vezom na serveru

Cody Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `cody.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, Cody Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Cody karakteristikama.

---

### Obavještenja se ne prikazuju

Cody Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Cody u postavkama vašeg OS-a, i
...
1. Zatvorite Cody Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Cody Desktop aplikacije):

- `cody.settings.dat` (URL zadanog servera za desktop)
- `cody.global.dat` i `cody.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `cody upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `cody models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\cody`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Cody dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\cody`

2. Ponovo pokrenite Cody da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Cody da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Cody će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
