---
title: Custom Tools
description: Create tools the LLM can call in cody.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside cody's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Or globally, by placing them in `~/.config/cody/tools/`.
