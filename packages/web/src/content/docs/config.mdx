---
title: Config
description: Using the Cody JSON config.
---

You can configure Cody using a JSON config file.

---

## Format

Cody supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="cody.jsonc"
{
...
1. **Remote config** (from `.well-known/cody`) - organizational defaults
2. **Global config** (`~/.config/cody/cody.json`) - user preferences
3. **Custom config** (`CODY_CONFIG` env var) - custom overrides
4. **Project config** (`cody.json` in project) - project-specific settings
5. **`.cody` directories** - agents, commands, plugins
6. **Inline config** (`CODY_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/cody/` on macOS) - admin-controlled
...
:::note
The `.cody` and `~/.config/cody` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/cody` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/cody"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Cody config in `~/.config/cody/cody.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/cody/tui.json`.
...
Add `cody.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Cody starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
modes, and plugins just like the standard `.cody` directory, and should
...
The custom directory is loaded after the global config and `.cody` directories, so it **can override** their settings.
...
Drop an `cody.json` or `cody.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/cody/` |
| Linux    | `/etc/cody/`                         |
| Windows  | `%ProgramData%\cody`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Cody reads managed preferences from the `ai.cody.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Cody checks these paths:

1. `/Library/Managed Preferences/<user>/ai.cody.managed.plist`
2. `/Library/Managed Preferences/ai.cody.managed.plist`

The plist keys map directly to `cody.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.cody.managed` PayloadType. The Cody config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.cody.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.cody.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.cody</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`cody.ai/config.json`**](https://cody.dev/config.json).

TUI config uses [**`cody.ai/tui.json`**](https://cody.ai/tui.json).
...
Legacy `theme`, `keybinds`, and `tui` keys in `cody.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `cody serve` and `cody web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Cody server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `cody.local`. Useful for running multiple instances on the same network.
...
If not specified, Cody will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Cody config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Cody tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
You can also define agents using markdown files in `~/.config/cody/agents/` or `.cody/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Cody will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`cody run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/cody/commands/` or `.cody/commands/`. [Learn more here](/docs/commands).
...
Cody uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Cody will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, cody **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Cody with custom tools, hooks, and integrations.

Place plugin files in `.cody/plugins/` or `~/.config/cody/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="cody.json"
{
  "$schema": "https://cody.dev/config.json",
  "plugin": ["cody-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Cody to only use specific providers rather than disabling them one by one.
