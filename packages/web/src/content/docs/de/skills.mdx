...
Agent Skills erlauben Cody, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Cody sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Cody vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
...
Globale Definitionen kommen zusaetzlich aus `~/.config/cody/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Cody listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `cody.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `cody.json`):
