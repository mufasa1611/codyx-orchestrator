...
Mit Cody kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Cody das `cody`-Theme.
...
Cody liefert mehrere eingebaute Themes mit.
...
- Cody optisch ans Terminal anpassen wollen
...
Cody unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/cody/themes/*.json` oder `$XDG_CONFIG_HOME/cody/themes/*.json`
