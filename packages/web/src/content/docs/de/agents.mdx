...
Es gibt zwei Arten von Agenten in Cody; Primäragenten und Subagenten.
...
Cody verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `cody.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Cody modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Cody-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Cody-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Cody Zen](/docs/zen) verwenden, würden Sie `cody/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `cody models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
