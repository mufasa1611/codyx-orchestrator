...
Cody bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in Cody verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn der Cody-Provider genutzt wird oder die Umgebungsvariable `CODY_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von Cody:
