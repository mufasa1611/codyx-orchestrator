---
title: Regeln
description: Lege eigene Anweisungen fuer cody fest.
---

Du kannst cody ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in cody den Befehl `/init` aus.
...
So kann cody sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht cody sie zu erweitern.
...
cody kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/cody/AGENTS.md` speichern.
Diese gelten in allen cody-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Cody dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/cody/AGENTS.md` existiert)
...
Beim Start sucht cody in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/cody/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `cody.json` oder in `~/.config/cody/cody.json` angeben.
...
cody parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit cody.json

Empfohlen ist das `instructions`-Feld in der `cody.json`:
...
Du kannst cody explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `cody.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
