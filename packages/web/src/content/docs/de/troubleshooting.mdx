...
Wenn Cody Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\cody\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `cody --log-level DEBUG`.

---

## Speicher

cody speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/cody/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\cody`
...
Cody Desktop startet im Hintergrund einen lokalen Cody-Server (`cody-cli`-Sidecar).
...
- Nur macOS: `Cody`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/cody/cody.jsonc` (oder `~/.config/cody/cody.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/cody/cody.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\cody\cody.jsonc`
...
Cody kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/cody/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\cody\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Cody ihn neu aufbauen kann.

1. Beende Cody Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/cody`
- **Linux**: loesche `~/.cache/cody` (oder fuehre aus: `rm -rf ~/.cache/cody`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\cody`

3. Starte Cody Desktop neu.

---

### Server-Verbindungsprobleme beheben

Cody Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `cody.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt Cody Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Cody-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Cody Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Cody in den OS-Einstellungen aktiviert sind, und
...
1. Beende Cody Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Cody Desktop):

- `cody.settings.dat` (desktop default server URL)
- `cody.global.dat` und `cody.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Cody hast:
...
3. Stelle sicher, dass du die neueste Version hast: `cody upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `cody models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\cody`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. cody installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\cody`

2. Starte cody neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt cody, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
cody erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
