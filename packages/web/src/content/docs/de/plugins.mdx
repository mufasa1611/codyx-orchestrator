---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Cody zu erweitern.
---

Mit Plugins können Sie Cody erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Cody zu ändern.
...
- `~/.config/cody/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/cody/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/cody/cody.json`)
2. Projektkonfiguration (`cody.json`)
3. Globales Plugin-Verzeichnis (`~/.config/cody/plugins/`)
...
Cody führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Cody-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Cody erweitern können.
...
:::note
Wenn Sie die Cody-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Cody `.env`-Dateien liest:
...
Plugins können Cody auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Cody aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Cody zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://cody.ai/docs/sdk).
