---
title: Server
description: Nutze den cody-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `cody serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein cody-Client nutzen kann.
...
Der Benutzername ist standardmaessig `cody`, kann aber mit `CODY_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `cody serve` und `cody web`.
...
Wenn du `cody` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den cody-Server, um cody programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `cody serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `cody serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Cody-[IDE](/docs/ide)-Plugins.
...
Der cody-Server stellt folgende APIs bereit.
