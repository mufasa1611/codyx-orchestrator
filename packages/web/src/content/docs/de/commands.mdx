...
Sie können benutzerdefinierte Befehle über die Cody-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Cody [config](/docs/config):

```json title="cody.jsonc" {4-12}
{
...
- Global: `~/.config/cody/commands/`
- Pro Projekt: `.cody/commands/`

```markdown title="~/.config/cody/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
Cody enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
