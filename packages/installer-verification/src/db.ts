...
export async function ensureSchema(db: D1Database) {
  ...
  try {
    await db.prepare("ALTER TABLE registration ADD COLUMN session_started_at INTEGER DEFAULT 0").run()
  } catch {}
...
export async function cleanup(db: D1Database, now = Date.now()) {
  ...
  return db.batch([
    ...
    db.prepare("DELETE FROM feedback WHERE retain_until < ?").bind(now),
  ])
}
